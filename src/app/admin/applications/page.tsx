...
export default function ApplicationsPage() {
  ...
  async function handleRestore(id: string) {
    try {
      await adminApi.updateApplication(id, { restore: true });
      load();
    } catch (err: unknown) {
  ...
  async function runBulk(action: string) {
    ...
    setBulkMsg('');
    try {
      const result = await adminApi.bulkApplications([...checked], action, action === 'update_status' ? bulkStatus : undefined);
  ...
  function doExport() {
    ...
    if (search) params.search = search;
    if (statusFilter) params.status = statusFilter;
    adminApi.exportApplications(Object.keys(params).length ? params : undefined);
  }
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: selected ? '1px solid var(--line)' : 'none' }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--line)', display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={doExport} style={btnSecondary} title="Export current view as CSV">CSV</button>
        </div>
        ...
        {checked.size > 0 && (
          <div style={{ padding: '8px 20px', borderBottom: '1px solid var(--line)', background: 'var(--accent-soft)', display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {showTrash && (
              <button disabled={bulkLoading} onClick={() => runBulk('restore')} style={btnPrimary}>Restore all selected</button>
            )}
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            ...
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--fg-4)', fontSize: 13 }}>
              {showTrash ? 'Trash is empty' : 'No applications found'}
            </div>
          ) : (
                ...
                {items.map(a => (
                  <tr
                    ...
                    style={{
                      ...
                      background: selected?._id === a._id ? 'var(--accent-soft)' : checked.has(a._id) ? 'rgba(99,102,241,0.06)' : 'transparent',
                    }}
                  >
                    <td style={{ ...td, width: 40 }} onClick={e => e.stopPropagation()}>
                      <input type="checkbox" checked={checked.has(a._id)} onChange={() => toggleCheck(a._id)} style={{ cursor: 'pointer' }} />
                    </td>
                    <td style={td} onClick={() => openDetail(a)}>{a.name}</td>
                    <td style={{ ...td, color: 'var(--fg-3)' }} onClick={() => openDetail(a)}>{a.role}</td>
                    <td style={{ ...td, color: 'var(--fg-3)', fontSize: 12 }} onClick={() => openDetail(a)}>{a.email}</td>
                    <td style={{ ...td, color: 'var(--fg-4)', fontSize: 12, whiteSpace: 'nowrap' }} onClick={() => openDetail(a)}>{fmt(a.createdAt)}</td>
                    <td style={td} onClick={() => openDetail(a)}><StatusBadge status={a.status} /></td>
                    <td style={td} onClick={e => e.stopPropagation()}>
                      {showTrash ? (
                        <button onClick={() => handleRestore(a._id)} style={{ ...btnSecondary, padding: '4px 10px', fontSize: 12 }}>Restore</button>
                      ) : (
                        <button onClick={() => setDeleteConfirm(a._id)} aria-label="Move to trash" style={btnDanger}>Del</button>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      {selected && (
        <div style={{ width: 360, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            {([['Email', selected.email], ['Phone', selected.phone], ['Portfolio', selected.portfolio], ['Submitted', fmt(selected.createdAt)]] as [string, string][]).map(([l, v]) => v ? (
              <div key={l}>
            ...
            {!selected.deletedAt && (
              <>
                ...
                <div>
                  ...
                  <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} style={{ ...inputSm, width: '100%', resize: 'vertical', fontFamily: 'inherit', lineHeight: 1.5 }} placeholder="Internal notes…" />
                </div>
              </>
            )}
          ...
          <div style={{ padding: '12px 20px', borderTop: '1px solid var(--line)', display: 'flex', gap: 8 }}>
            {selected.deletedAt ? (
              ...
              <>
                <button onClick={saveChanges} disabled={saving} style={{ ...btnPrimary, flex: 1 }}>{saving ? 'Saving…' : 'Save changes'}</button>
      ...
      {deleteConfirm && (
        <div style={modalOverlay}>
          <div style={modalBox}>
            ...
            <div style={{ fontSize: 13, color: 'var(--fg-3)', marginBottom: 20 }}>The application will be moved to trash and can be restored later.</div>
...
const thStyle: React.CSSProperties = {
  padding: '10px 14px', fontSize: 11, fontWeight: 600, color: 'var(--fg-4)',
...
const btnSecondary: React.CSSProperties = {
  padding: '7px 12px', fontSize: 13, fontWeight: 500, color: 'var(--fg-2)',
